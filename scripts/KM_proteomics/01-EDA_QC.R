...
dat = readxl::read_xlsx('../../raw_data/multiomics/TK_KidneyMarrowAndPlasma_proteomics_metabolomics/kidneyMarrow_proteomics.xlsx',
                        col_names = c('Protein','Age',paste('s',1:5, sep = '')), skip = 1) %>%
  na.omit()  %>%
  mutate(Age = ifelse(Age == 'Old','old','young'))
...
plotsave(allsamples_boxp, './results/KMproteomics/allsamples/exp_dist_box', width = 12, height = 6)


# correlations
pheatmap::pheatmap(cor(mat_l2), cellwidth = 10, cellheight = 10, color = RColorBrewer::brewer.pal(8,'Oranges')[-1], filename = './results/KMproteomics/allsamples/correlationplot.pdf')
pheatmap::pheatmap(cor(mat_l2), cellwidth = 10, cellheight = 10, color = RColorBrewer::brewer.pal(8,'Oranges')[-1], filename = './results/KMproteomics/allsamples/correlationplot.png')
#young sample2 is distant to all but has correlation value above 0.95
...
#young s 2 falls distant on y axis. let's check the proteins
...
gores %>%
  filter(BY_corrected_p<=0.1) %>%
  View()

gores %>%
  tablesave('./results/KMproteomics/allsamples/outlierPC2_GOres')

objsave(mat, './data/processed/KMproteomics/expression')
objsave(mat_l2, './data/processed/KMproteomics/expression_log2')

#### repeat the same after excluding the outlier. 

mat = mat[,!colnames(mat)%in%c('young_s2')]
mat_l2 = mat_l2[,!colnames(mat_l2)%in%c('young_s2')]
...
plotsave(outremoved_boxp, './results/KMproteomics/outlier_removed/exp_dist_box', width = 12, height = 6)


# correlations
pheatmap::pheatmap(cor(mat_l2), cellwidth = 10, cellheight = 10, color = RColorBrewer::brewer.pal(8,'Oranges')[-1], filename = './results/KMproteomics/outlier_removed/correlationplot.pdf')
pheatmap::pheatmap(cor(mat_l2), cellwidth = 10, cellheight = 10, color = RColorBrewer::brewer.pal(8,'Oranges')[-1], filename = './results/KMproteomics/outlier_removed/correlationplot.png')
...
pcaplot = data.frame(pcx$x ) %>%
  ...
  xlab(imptext[1]) +
  ylab(imptext[2]) +
  theme_rvis(base_size = 6, legend.pos = c(0.5,1), just.par = c(0.5,1)) +
  coord_fixed(ratio = imp[2]/imp[1])
pcaplot

plotsave(pcaplot, './results/KMproteomics/outlier_removed/pca', width = 8, height = 8)
...
plotsave(pcaplot, './results/KMproteomics/outlier_removed/pca_noratio', width = 8, height = 8)

objsave(mat, './data/processed/KMproteomics/expression_outlierremoved')
objsave(mat_l2, './data/processed/KMproteomics/expression_log2_outlierremoved')
