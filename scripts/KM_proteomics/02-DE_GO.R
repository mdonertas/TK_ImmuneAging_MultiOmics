...
mat_l2 = readRDS('./data/processed/KMproteomics/expression_log2.rds')

# wiltest = data.frame(t(apply(mat_l2,1, function(x){
#   y = x[grepl('young',names(x))]
#   o = x[grepl('old',names(x))]
#   l2fc = median(o) - median(y)
#   difs = c(sapply(o,function(xx){sapply(y,function(z)xx-z)}))
...
#   tablesave('./results/KMproteomics/allsamples/proteinDEtest')
# 
# objsave(wiltest,'./data/processed/KMproteomics/DEwilcox_allsamples')
...
# objsave(gores,'./data/processed/KMproteomics/DE_GOenrich_allsamples')
...
gores %>%
  ...
  tablesave('./results/KMproteomics/allsamples/GOenrichment')
...
gogeneplots = lapply(unique(repgenedat$`GO representative`), function(gorep){
  repgenedat %>%
    ...
    guides(fill = 'none') +
    geom_text(data = . %>% filter(padj<=0.1) %>% mutate(labx = ifelse(abs(Q3)>abs(Q1),Q3+0.1,Q1-0.1)), aes(x = labx), label='*', vjust = 0.8, size = 8/pntnorm, color = 'gray35')
})
names(gogeneplots) = unique(repgenedat$`GO representative`)

sapply(names(gogeneplots), function(gorep){
  fname = paste('./results/KMproteomics/allsamples/GOgenes/',gsub('/','_',gsub(' ', '_',gorep)),sep='')
  plotsave(gogeneplots[[gorep]],fname,width = 8, height = max(4,length(unique(gogeneplots[[gorep]]$data$protein))/3))
})


drivergene_plot_pos = repgenedat %>%
  # filter(`GO representative` == gorep) %>%
  select(-1) %>%
  unique() %>%
  filter((l2FC) >= 1) %>%
...
plotsave(
  drivergene_plot_pos,
  './results/KMproteomics/allsamples/drivergenes_positive',
  width = 8,
  height = 26
)

drivergene_plot_neg = repgenedat %>%
  # filter(`GO representative` == gorep) %>%
  select(-1) %>%
  unique() %>%
  filter((l2FC) <= -1) %>%
...
plotsave(
  drivergene_plot_neg,
  './results/KMproteomics/allsamples/drivergenes_negative',
  width = 8,
  height = 26
)
...
plotsave(
  drivergene_plot,
  './results/KMproteomics/allsamples/drivergenes',
  width = 8,
  height = 10
)
...
wiltest2 %>% select(upperprotein) %>% unique() %>% nrow()
# 6770
wiltest2 %>% select(humangene) %>% unique() %>% nrow()
# 5208
wiltest2 %>% select(upperprotein, humangene) %>% na.omit() %>% unique()  %>% nrow()
# 5395
...
allortgores = readRDS('./data/processed/KMproteomics/DE_GOenrich_allsamples.rds') %>%
  # filter(BY_corrected_p<=0.05) %>%
